...
const RESERVED_TABLE_NAMES = ['botmate_migrations', 'botmate_database_schema'];
...
module.exports = (db) => {
  ...
  const diffSchemas = async (srcSchema, destSchema) => {
    ...
    const persistedTables = helpers.hasTable(srcSchema, 'botmate_core_store_settings')
      ? (await botmate.store.get({
          type: 'core',
