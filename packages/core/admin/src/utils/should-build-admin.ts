...
const hasNonDefaultPlugins = (plugins) => {
	...
	const installedPlugins = Object.keys(plugins).filter(
		(x) => !DEFAULT_PLUGINS.includes(x)
	);

	// List of default plugins uninstalled from a generated app
	const missingPlugins = DEFAULT_PLUGINS.filter(
		(x) => !Object.keys(plugins).includes(x)
	);
...
const hasCustomAdminCode = async (dir) => {
	...
	const customAdminWebpackFile = path.join(
		customAdminPath,
		'webpack.config.js'
	);
