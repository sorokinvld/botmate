 ...
 */
async function watchAdminFiles(dir: string) {
	const useTypeScript = await isUsingTypeScript(
		path.join(dir, 'src', 'admin'),
		'tsconfig.json'
	);
	...
	watcher.on('all', async (event, filePath) => {
		...
		const targetPath = isAppFile
			? path.join(
					path.normalize(filePath.split(appExtensionFile)[1]),
					targetExtensionFile
			  )
			: path.join(
					'extensions',
					path.normalize(filePath.split(extensionsPath)[1])
			  );
...
export default watchAdminFiles;
